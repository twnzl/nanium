...
describe('queue entry: calculate next run', () => {
	...
	beforeAll(() => {
		(global as any).Date.now = (mockDate).getTime.bind(mockDate);
	});
	...
	test('recurring: each year on each Monday in January and April at 10:05:00', async () => {
		...
		const result = Nanium.calculateNextRun(entry, lastRun);
	...
	test('recurring: if second is * it should never calculate the same second as lastRun', async () => {
		...
		const result = Nanium.calculateNextRun(entry, lastRun); // via date mock the current time is same as lastRun
